...
def imag(image_n,image_f):
    
    image_data = image_f.read()
    n= np.frombuffer(image_data, np.uint8)
    img = cv2.imdecode(n, cv2.IMREAD_COLOR)
    
    d='static/image'
    os.makedirs(d, exist_ok=True)  # Create directory if it doesn't exist
    dest = os.path.join(d, image_n + '.jpg')
    ...
    destination = os.path.join(destination_dir, image_n + '.png')
